...
if(!empty($_POST)){
    ...
    if (isset($_POST['avatar'])){   // On se positionne sur le bon formulaire
        $playerInformation['player_avatar'] = $_POST['avatar'];
            ...
            if($width_orig >= 250 && $height_orig >= 250 && $width_orig <= 6000 && $height_orig <= 6000){ // On vérifie que la taille de l'image et correcte
                ...
                if ($_FILES['file']['size'] <= $tailleMax){ // Si le fichier et bien de taille inférieur ou égal à 5 Mo
                    ...
                    if (in_array($extensionUpload, $extensionsValides)){ // Vérifie que l'extension est correct
                        ...
                        $dossier = "../assets/img/avatar/" . $_SESSION['user']['id'] . "/" . $playerInformation['player_avatar'] . "/"; // On se place dans le dossier de la personne 
                        ...
                        if (!is_dir($dossier)){ // Si le nom de dossier n'existe pas alors on le crée
                            ...
                            if(file_exists("../assets/img/avatar/". $_SESSION['user']['id'] . "/" . $playerInformation['player_avatar']) && isset($playerInformation['player_avatar'])){
                                unlink("../assets/img/avatar/". $_SESSION['user']['id'] . "/" . $playerInformation['player_avatar']);
                               
                            }
                        }

                        $nom = md5(uniqid(rand(), true)); // Permet de générer un nom unique à la photo
                        $chemin = "../assets/img/avatar/" . $_SESSION['user']['id'] . "/" . $playerInformation['player_avatar'] . "/" . $nom . "." . $extensionUpload; // Chemin pour placer la photo
                        ...
                        if($resultat) {
                            ...
                            $playerInformation['player_avatar'] = $chemin;
                        }
